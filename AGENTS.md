...
You are assisting with Gooble. A flexible and robust Python3 based chatbot for my Mumble server.
...
Optional: run as a systemd service (Linux). Example unit file `/etc/systemd/system/gooble.service`:

```ini
[Unit]
Description=Gooble Mumble Bot
After=network.target

[Service]
User=gooble
WorkingDirectory=/path/to/Gooble
ExecStart=/path/to/Gooble/venv/bin/python3 /path/to/Gooble/main.py
Restart=on-failure
